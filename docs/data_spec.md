...
## Resonet Multi-Frame CXI Format

CXI files generated by the Resonet simulation pipeline differ from the one-file-per-frame convention used elsewhere. Key properties:

| Property | Value |
|----------|-------|
| HDF5 data key | `entry_1/data_1/data` |
| Frame shape | `(N, 5632, 384)` — N frames per file |
| Label key | `entry_1/labels/hit` |
| Label encoding | `float32`: `1.0` = hit (class 1), `0.0` = non-hit (class 0) |
| Detector metadata | `entry_1/instrument_1/detector_1/description` = `"EIGER 4M"` |
| Compression | gzip level 4, chunked `(1, 5632, 384)` |

**Geometry note:** The (5632, 384) frame layout does not match the standard Reborn Eiger4M geometry (2167×2070). Full panel assembly is deferred until the correct Reborn geometry for this readout mode is confirmed. Use `preprocess_assembled()` (bypasses geometry) for normalization in the interim.

**Dataset class:** Use `MultiFrameCXIDataset` from `src/data/dataset.py`. It expands all CXI files into a flat `(file_path, frame_idx)` index, reads embedded labels, and applies `preprocess_assembled` by default.
