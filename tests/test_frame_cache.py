...
def _fake_jungfrau_assembly(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    detector-agnostic stand-in for generic frame-cache logic — real
    PADAssembler requires a full-size (2164, 2068) canvas. Fall back to the
    old _to_2d() passthrough only for undersized synthetic frames;
    TestBitExactnessAgainstRealData uses real-shaped frames and still goes
...
class TestLodoWiring:
    def test_verify_cache_or_raise_accepts_none(self) -> None:
        from src.data.frame_cache import verify_cache_or_raise

        # No cache configured — must be a silent no-op, not an error.
        verify_cache_or_raise(None, {})
    ...
    def test_verify_cache_or_raise_passes_on_matching_manifest(
        ...
        root.mkdir()
        write_manifest(root, CFG)
        verify_cache_or_raise(FrameCache([root]), CFG)
    ...
    def test_verify_cache_or_raise_rejects_stale_manifest(self, tmp_path: Path) -> None:
        ...
        with pytest.raises(CacheStaleError, match="pf8_min_snr"):
            verify_cache_or_raise(FrameCache([root]), changed)
    ...
    def test_verify_cache_or_raise_rejects_root_without_manifest(
        ...
        with pytest.raises(CacheStaleError, match="no cache manifest"):
            verify_cache_or_raise(FrameCache([empty]), CFG)
    ...
    def test_verify_cache_or_raise_tolerates_unbuilt_nvme_tier(
        ...
        verify_cache_or_raise(FrameCache([nvme, nfs]), CFG)
    ...
    def test_train_fold_forwards_cache_to_every_call_site(self) -> None:
        ...
        assert "verify_cache_or_raise(frame_cache, cfg)" in src
