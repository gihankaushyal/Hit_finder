...
| 14 | Peak-aware cutout, holes 32→24 px (2026-08-17) | Uniform cutout occluded the anchor Bragg peak on ~8% of hit crops (label noise: image says hit, evidence erased). `random_cutout(..., avoid=)` rejection-samples hole positions with 8 px margin from all hitfinder centroids (skip after 20 tries). Peak centroids ride as a third stacked channel `(H,W,3)` through crop/rot90/flip — no coordinate transforms. Training order changed same day to rot90→flip→**LCN→cutout** (user decision): cutout holes are exact 0 in LCN space and never enter local stats; training now matches the debug runner exactly. |
